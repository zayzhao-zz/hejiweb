...
export default function SalaryView(){
  ...
  function formatNumber(v){ return v == null || v === '' ? '' : Number(v).toLocaleString() }
  ...
  async function fetchData(opts = {}){
    ...
    try{
      ...
      console.error('fetch salary failed', err)
      setOriginalRows([]); setRows([]); setFetchCount(0)
    }finally{ setLoading(false) }
  }

  // 不在挂载时自动加载，需点击“应用”以加载数据（和其他页面一致）
  ...
  return (
    <div>
      <h3 className="text-2xl font-semibold mb-4">员工薪资信息</h3>
          ...
          <table className="min-w-full text-left table-auto">
            <thead>
              <tr className="border-b">
                ...
                <th className="px-3 py-2 text-right">工资</th>
                <th className="px-3 py-2 text-right">前厅出勤天数</th>
                <th className="px-3 py-2 text-right">厨房出勤天数</th>
                <th className="px-3 py-2 text-left">人效价值</th>
              </tr>
            </thead>
            <tbody>
              {rows.map(r=> (
                <tr key={r.id} className="border-b odd:bg-white even:bg-slate-50 dark:odd:bg-zinc-900/60 dark:even:bg-zinc-800/60 hover:bg-slate-100 dark:hover:bg-zinc-700">
                  ...
                  <td className="px-3 py-2 text-right">{formatCurrency(r['工资'])}</td>
                  <td className="px-3 py-2 text-right">{formatNumber(r['前厅出勤天数'])}</td>
                  <td className="px-3 py-2 text-right">{formatNumber(r['厨房出勤天数'])}</td>
                  <td className="px-3 py-2">{r['人效价值']}</td>
                </tr>
              ))}
            </tbody>
          </table>
